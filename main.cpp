...
void Tree::delete_value(value* key) {
	...
	while (true) {
		...
		if (i < q->num) {
			r = q->n[i + 1];
			r->v[1] = r->v[0];
			r->n[2] = r->n[1]; r->n[1] = r->n[0]; r->n[3] = nullptr;
			r->v[0] = q->v[i]; r->v[2] = nullptr;
			if (p->n[0] != nullptr) r->n[0] = p->n[0];
			if (p->n[1] != nullptr) r->n[0] = p->n[1];
			r->num++;
			for (j = i; j < q->num - 1; j++) {
				q->v[j] = q->v[j + 1];
				q->n[j] = q->n[j + 1];
...
void main() {
	...
	while (true) {
		cout << "\nOdaberite opciju\n";
		cout << "1.Unesi novi kljuc u stablo\n";
		cout<< "2.Obrisi kljuc iz stabla\n";
		cout<< "3.Pretrazi stablo\n";
		cout<< "4.Prebroj akcije po prioritetu\n";
		cout<< "5.Promeni prioritet akcije\n";
		cout<< "6.KRAJ!\n";
		cin >> i;
		if (i == 1) {
		...
		if (i == 4) {
			cout << "Unesite vrednost\n";
			cin >> n;
			cout << "Ima " << t.count(n) << " operacija sa vrednoscu " << n;
		}
		if (i == 5) {
			cout << "Unesite vrednost\n";
			cin >> n >> s;
...
/*	value* v1 = new value, * v2 = new value, * v3 = new value, * v4 = new value, * v5 = new value, * v6 = new value, * v7 = new value, * v8 = new value, * v9 = new value, * v0 = new value;
	v1->p = 1; v1->s = "aa";
	v2->p = 2; v2->s = "aa";
	v3->p = 3; v3->s = "aa";
	v4->p = 4; v4->s = "aa";
